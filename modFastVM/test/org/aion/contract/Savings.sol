...
contract Savings is Owned {
	...
	*/
	...
	function bulkDepositTo(uint128[] bits) onlyOwner {
		uint128 lomask = (1 << 96) - 1;
	...
	function _withdrawTo(uint _deposit, uint _withdrawn, uint _blockTimestamp, uint _total) constant returns (uint) {
		 ...
		 *
		 * The maximum for a uint128 is = 1.15 * (10 ** 77)
		 */
		uint128 withdrawable = ((_deposit * fraction * _total) / totalfv) / precision;
	...
	function multiMint(uint nonce, uint128[] bits) onlyOwner preLock {
